...
const StageComponent = ({ stage, onSolved, audioEnabled, theme, canAdvance, isStageComplete, onGoToPreviousStage, onGoToNextStage, stagesSolved, totalStages }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (!allEvidenceReviewed) {
      announceToScreenReader('⚠️ You must examine all evidence before presenting your findings. Click on each piece of evidence to review it thoroughly.');
      return;
    }
...
// (EvidenceComponent, PuzzleInterface, helper functions, etc.)
...
const EvidenceComponent = ({ evidence, audioEnabled, theme, onEvidenceReviewed, reviewedEvidence }) => {
  ...
  const handleDocumentReview = () => {
    if (!reviewedEvidence.has('document')) {
      onEvidenceReviewed('document');
    }
  };

  const handleAudioReview = () => {
    if (!reviewedEvidence.has('audio')) {
      onEvidenceReviewed('audio');
    }
  };
  ...
  switch (evidence.type) {
    case 'crime-scene':
      return (
        <div className="space-y-4">
          ...
          <div className="text-sm text-gray-400 bg-slate-600/30 rounded-lg p-3">
            <p className="flex items-center gap-2">
              <Eye className="w-4 h-4" />
              Click on each piece of evidence to examine it thoroughly and mark it as reviewed
            </p>
          </div>
        </div>
      );
      ...
      return (
        <div className="space-y-4">
          <div className="bg-slate-700 rounded-lg p-4">
            ...
            <div className="space-y-4">
              {evidence.phenomena.map((phenomenon, index) => (
                <motion.div
                  key={index}
                  ...
                  className={`cursor-pointer p-4 rounded-lg transition-all duration-300 border-2 ${
                    reviewedEvidence.has(index)
                      ...
                      : 'bg-slate-600/30 border-purple-500/20 hover:bg-slate-600/50'
                  }`}
                  onClick={() => handleEvidenceClick(index)}
                >
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <h6 className="font-semibold text-purple-300">{phenomenon.name}</h6>
                    {reviewedEvidence.has(index) && (
                      <span className="text-xs bg-green-500/20 text-green-300 px-2 py-1 rounded">
                        ✓ Examined
                      </span>
                  ...
                  <p className="text-gray-300 text-sm">{phenomenon.description}</p>
                  <AnimatePresence>
                    {selectedEvidence === index && (
                      <motion.div
                        initial={{ opacity: 0, height: 0 }}
                        animate={{ opacity: 1, height: 'auto' }}
                        exit={{ opacity: 0, height: 0 }}
                        className="mt-3 p-3 bg-purple-500/10 rounded-lg"
                      >
                        {phenomenon.investigation && (
                          <p className="text-purple-200 text-sm mb-2">{phenomenon.investigation}</p>
                        )}
                        {phenomenon.spirit_message && (
                          <div className="bg-purple-500/20 border border-purple-500/30 rounded p-2">
                            <span className="font-medium text-purple-300">Spirit Message: </span>
                            <span className="text-purple-100 italic">"{phenomenon.spirit_message}"</span>
                          </div>
                        )}
                      </motion.div>
                    )}
                ...
                <motion.div
                  ...
                  className={`cursor-pointer p-4 rounded-lg transition-all duration-300 border-2 ${
                    reviewedEvidence.has(index)
                      ...
                      : selectedEvidence === index
                      ? 'bg-blue-500/20 border-blue-500/40 hover:bg-blue-500/25'
                      : 'bg-slate-600/30 border-blue-500/20 hover:bg-slate-600/50'
                  }`}
                  onClick={() => handleEvidenceClick(index)}
                >
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <h6 className="font-semibold text-blue-300">{artifact.name}</h6>
                    {reviewedEvidence.has(index) && (
                      <span className="text-xs bg-green-500/20 text-green-300 px-2 py-1 rounded">
                        ✓ Examined
                      </span>
                  ...
                  <AnimatePresence>
                    {selectedEvidence === index && (
                      ...
                      >
                        {artifact.magical_property && (
                          <div className="mb-2">
                            <span className="font-medium text-blue-300">Magical Property: </span>
                            <span className="text-blue-200">{artifact.magical_property}</span>
                          </div>
                        )}
                        {artifact.inscription && (
                          <div className="bg-blue-500/20 border border-blue-500/30 rounded p-2">
                            <span className="font-medium text-blue-300">Ancient Inscription: </span>
                            <span className="text-blue-100 italic">"{artifact.inscription}"</span>
                          </div>
                        )}
                      </motion.div>
                    )}
    ...
    case 'document':
      return (
        <div className="space-y-4">
          <div 
            className={`bg-slate-700 rounded-lg p-4 cursor-pointer transition-all duration-300 border-2 ${
              reviewedEvidence.has('document')
          ...
          >
            <h5 className="font-semibold text-white mb-4 flex items-center gap-2">
              ...
              {evidence.title}
              {reviewedEvidence.has('document') && (
                <span className="text-xs bg-green-500/20 text-green-300 px-2 py-1 rounded ml-2">
                  ✓ Read
                </span>
              )}
            </h5>
            <div className="bg-slate-600 rounded-lg p-4 font-mono text-sm text-gray-300 leading-relaxed">
              {evidence.content}
            </div>
            {!reviewedEvidence.has('document') && (
              <p className="text-yellow-300 text-sm mt-3 flex items-center gap-2">
                <Eye className="w-4 h-4" />
                Click to read and mark this document as reviewed
              </p>
            )}
          </div>
        </div>
      ...
      return (
        <div className="space-y-4">
          <div className={`p-4 rounded-lg border-2 transition-all duration-300 ${
            reviewedEvidence.has('audio')
              ? 'border-green-500/30 bg-green-500/5'
              : 'border-transparent bg-slate-700/50'
          }`}>
            <div className="flex items-center gap-2 mb-3">
              <h5 className="font-semibold text-white">Audio Evidence</h5>
              {reviewedEvidence.has('audio') && (
                <span className="text-xs bg-green-500/20 text-green-300 px-2 py-1 rounded">
                  ✓ Listened
                </span>
            ...
            <audio
              ...
              onPlay={handleAudioReview}
            >
              <source src={evidence.src} type="audio/mpeg" />
              Your browser does not support the audio element.
            </audio>
            <div className="flex gap-2 mt-3">
              <button
                onClick={() => setShowTranscript(!showTranscript)}
                className="px-3 py-2 bg-blue-500/20 text-blue-300 rounded-lg hover:bg-blue-500/30 transition-colors text-sm flex items-center gap-2"
              >
            ...
            {showTranscript && (
              <div className="bg-slate-700 rounded-lg p-4 mt-3">
                <h6 className="font-semibold text-white mb-2">Audio Transcript</h6>
            ...
            {!reviewedEvidence.has('audio') && (
              <p className="text-yellow-300 text-sm mt-3 flex items-center gap-2">
                <Eye className="w-4 h-4" />
                Play the audio to mark it as reviewed
              </p>
            )}
          </div>
        </div>
  ...
  const puzzleLibrary = {
    'murder-mystery': {
      1: {
        ...
        evidence: {
          ...
          items: [
            ...
            {
              name: "Desk Papers",
              description: "Business documents scattered on desk",
              details: "Recent partnership agreements and a threatening letter from a business rival.",
              clue: "Letter mentions 'you'll pay for what you've done' - signed 'M.R.'"
            },
            {
              name: "Window",
              description: "Large window behind the desk, locked",
              details: "Window is locked from inside, no signs of forced entry.",
              clue: "No external entry possible - killer was already inside"
            }
          ]
        },
        options: [
      ...
      2: {
        ...
        type: "text-input",
        inputLabel: "Who is the killer?",
        placeholder: "Enter the killer's name",
        solution: "margaret-reynolds",
        evidence: {
          type: "document",
          title: "Witness Testimonies",
          content: `BUTLER (James): "I brought Lord Blackwood his morning tea at 9 AM as usual. He was alone in his study reading papers. I left immediately and didn't return until I found him at 11 AM."

BUSINESS PARTNER (Robert): "I arrived at 10 AM for our meeting. The butler said Lord Blackwood was busy, so I waited in the drawing room until 11 AM when we heard the commotion."

NIECE (Margaret Reynolds): "I visited Uncle yesterday evening around 8 PM. We had a terrible argument about his new business deal. I left angry and didn't return until this morning at 10:30 AM to apologize. I went straight to his study."

CORONER'S REPORT: Time of death estimated between 10:15-10:45 AM based on body temperature.`
        },
        hints: [
          "Compare each person's story with the time of death.",
          "Look for who had the opportunity to poison the tea.",
          "Remember the threatening letter signed 'M.R.'",
          "Margaret Reynolds' initials are M.R., and she had access during the crucial time window."
        ]
      },
      3: {
        ...
        type: "text-input",
        inputLabel: "Where did Margaret get the cyanide?",
        ...
        evidence: {
          ...
          items: [
            {
              name: "Margaret's Handbag",
              description: "Small leather purse with various personal items",
              details: "Contains makeup, keys, and a small glass vial with traces of white powder.",
              clue: "The vial has a photography supply store label"
            },
            {
              name: "Photography Equipment",
              description: "Camera and developing supplies in Margaret's room",
              details: "Professional darkroom setup with various chemicals for photo development.",
              clue: "Potassium cyanide is used in photo development processes"
            },
            {
              name: "Purchase Receipt",
              description: "Receipt from Eastman Photography Supplies",
              details: "Dated one week ago, shows purchase of various darkroom chemicals.",
              clue: "Includes 'KCN - Potassium Cyanide' for silver recovery"
            },
            {
              name: "Diary Entry",
              description: "Margaret's personal journal",
              details: "Recent entries show increasing anger about inheritance changes.",
              clue: "'He's going to cut me out completely. There has to be another way.'"
            }
          ]
        },
        hints: [
          "Look at Margaret's hobbies and interests.",
          "Consider legitimate uses for cyanide in everyday activities.",
          "Check the receipt for what Margaret purchased recently.",
          "Photography development uses potassium cyanide for silver recovery."
        ]
      },
      4: {
        ...
        type: "multiple-choice",
        inputLabel: "What evidence will convince Margaret to confess?",
        solution: "complete-timeline",
        evidence: {
          type: "document",
          title: "Case Summary",
          content: `EVIDENCE SUMMARY:
1. Lord Blackwood died from cyanide poisoning between 10:15-10:45 AM
2. Margaret Reynolds visited during this time window (10:30 AM)
3. Threatening letter signed "M.R." found at scene
4. Margaret had access to cyanide through photography supplies
5. Recent purchase receipt shows she bought potassium cyanide one week ago
6. Diary entries reveal motive: fear of being cut from inheritance
7. Only Margaret had motive, means, and opportunity during the critical time

TIMELINE:
- 9:00 AM: Butler serves tea, leaves study
- 10:00 AM: Robert arrives, waits in drawing room
- 10:30 AM: Margaret arrives, goes directly to study
- 10:45 AM: Lord Blackwood dies from cyanide poisoning
- 11:00 AM: Body discovered by butler`
        },
        options: [
          { value: "fingerprints-only", label: "Fingerprint Evidence", description: "Focus only on physical evidence" },
          { value: "motive-alone", label: "Inheritance Motive", description: "Emphasize only the financial motive" },
          { value: "complete-timeline", label: "Complete Case Timeline", description: "Present all evidence in chronological order" },
          { value: "witness-testimony", label: "Witness Statements", description: "Rely primarily on what others said" }
        ],
        hints: [
          "Consider what would be most convincing to a guilty person.",
          "A complete picture is harder to deny than partial evidence.",
          "The timeline shows how all pieces fit together.",
          "When presented with overwhelming evidence, most criminals confess."
        ]
      },
      5: {
        ...
        type: "text-input",
        inputLabel: "What is the appropriate charge?",
        placeholder: "Enter the legal charge",
        solution: "premeditated-murder",
        evidence: {
          type: "document",
          title: "Margaret's Confession",
          content: `CONFESSION STATEMENT - Margaret Reynolds:

"I admit it. I killed Uncle Blackwood. I couldn't let him change his will and leave me with nothing. I've been taking care of him for years while my cousins ignored him.

I bought the cyanide a week ago when I realized he was serious about cutting me out. I planned it carefully - I knew his routine, when the butler brought tea, when he would be alone.

This morning I went to apologize, but really I went to put the cyanide in his tea. I watched him drink it. I'm not sorry he's dead, but I'm sorry I got caught.

I planned this for a week. It wasn't a moment of anger - I wanted him dead."

Signed: Margaret Reynolds
Witnessed by: Detective Inspector Thompson
Date: [Current Date]`
        },
        hints: [
          "Consider the level of planning involved in the crime.",
          "Margaret purchased the poison a week in advance.",
          "She admits to planning the murder carefully.",
          "The legal term for planned murder is 'premeditated murder'."
        ]
      },
      6: {
        title: "Case Closed",
        ...
        type: "text-input",
        inputLabel: "What is the case resolution?",
        placeholder: "Enter the final outcome",
        solution: "trial-conviction",
        evidence: {
          type: "document",
          title: "Final Case Report",
          content: `CASE CLOSED - BLACKWOOD MURDER INVESTIGATION

VICTIM: Lord Blackwood (deceased)
PERPETRATOR: Margaret Reynolds (arrested)
CRIME: Premeditated Murder by Cyanide Poisoning

EVIDENCE:
- Cyanide residue in victim's tea
- Suspect's access to poison through photography supplies
- Written confession admitting to planning
- Timeline placing suspect at scene during time of death
- Financial motive (inheritance dispute)

RECOMMENDATION: Forward to Crown Prosecutor for trial proceedings.
...
Date: [Current Date]`
        ...
        hints: [
          ...
          "The next step is prosecution in court.",
          "A trial will determine her guilt and sentence.",
          "The case moves from investigation to prosecution."
        ]
      }
    },
